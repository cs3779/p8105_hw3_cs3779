...
The total number of aisles collected by _Instacart_ is `r summarize(instacart, count=n_distinct(aisle_id))`. The aisle that had the most items ordered from it was **`r aisle_count[1,]`** with *`r aisle_count[1,1]`* items bought, followed closely by `r aisle_count[2,]` with *`r aisle_count[2,2]`* items bought. 

This aisles that had over 10,000 items bought from them are shown in the below chart (organized from most to least):
```{r, echo=FALSE}
aisle_count %>% 
  filter(n>10000) %>% 
